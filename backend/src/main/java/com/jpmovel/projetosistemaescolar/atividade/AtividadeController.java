...
public class AtividadeController {
    ...
    public List<Atividade> listarTodas() {
        return atividadeRepository.findAllByOrderByDataLimiteEntregaAsc();
    }
    ...
    @PutMapping("/{id}")
    @PreAuthorize("hasAnyRole('COORDENADOR', 'PROFESSOR')")
    public ResponseEntity<Atividade> atualizar(@PathVariable Long id, @RequestBody @Valid Atividade dados) {
        Atividade atividade = atividadeRepository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("Atividade não encontrada com ID: " + id));
        Turma turma = turmaRepository.findByIdAndAtivoTrue(dados.getTurma().getId())
                .orElseThrow(() -> new ResourceNotFoundException("Turma não encontrada ou inativa"));
        Professor professor = professorRepository.findByIdAndAtivoTrue(dados.getProfessor().getId())
                .orElseThrow(() -> new ResourceNotFoundException("Professor não encontrado ou inativo"));

        atividade.setTitulo(dados.getTitulo());
        atividade.setDescription(dados.getDescription());
        atividade.setDataLimiteEntrega(dados.getDataLimiteEntrega());
        atividade.setTurma(turma);
        atividade.setProfessor(professor);
